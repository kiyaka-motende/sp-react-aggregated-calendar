...
import { Calendar, CommandBar, defaultCalendarStrings, getTheme, Stack, } from "@fluentui/react";
...
export var SpAggregatedCalendar = function (props) {
    ...
    var _o = React.useState([]), eventSourcesArray = _o[0], setEvents = _o[1];
    var _p = React.useState({ start: moment().startOf("month").toDate(), end: moment().endOf("month").toDate() }), viewDateRange = _p[0], setViewDateRange = _p[1];
    var _dataService = new spservices(props.context);
    React.useEffect(function () {
        function fetchEvents() {
            var myEvents = [];
            var promises = props.selectedCalendarLists.map(function (calendarData) {
                return _dataService.getEvents(escape(calendarData.SiteUrl), escape(calendarData.CalendarListTitle), viewDateRange.start, viewDateRange.end)
            ...
            React.createElement(Stack, { tokens: verticalGapStackTokens },
                ...
                React.createElement(Stack, { horizontal: true, horizontalAlign: "space-between" },
                    ...
                    React.createElement("div", null,
                        React.createElement(FullCalendar, { plugins: [
                                timeGridPlugin,
                                dayGridPlugin,
                                rrulePlugin,
                                listPlugin,
